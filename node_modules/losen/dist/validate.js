...
__webpack_require__(5).inherits(Command, EventEmitter);
...
if (!errors.length) {
  ...
  errors.forEach(function (_ref) {
    ...
    console.log((0, _chalk2.default)(_templateObject, path.join('.'), id ? '(' + id + ')' : '', error));
  });
}
...
var _dsl = __webpack_require__(21);
...
var requiredProperties = {
  ...
  Branch: ['branches'],
  Reference: ['nodeId'],
  Checkbox: [].concat(inputRequiredProperties, ['options']),
  Radio: [].concat(inputRequiredProperties, ['options']),
  Select: [].concat(inputRequiredProperties, ['options']),
...
function assertProperties(object, path, properties) {
  ...
  (properties || []).forEach(function (property) {
    if (typeof object[property] === 'undefined') {
      errors = [].concat(_toConsumableArray(errors), [{ path: path, id: object.id, error: object.type + ' is missing the \'' + property + '\' property' }]);
    }
  });
...
function validateNode(node, path, ancestors) {
  ...
  errors = [].concat(_toConsumableArray(errors), _toConsumableArray(assertProperties(node, path, requiredProperties[node.type])), _toConsumableArray(assertDeprecations(node, path, deprecatedProperties[node.type])));
  ...
  if (node.options) {
    node.options.forEach(function (option, index) {
      errors = [].concat(_toConsumableArray(errors), _toConsumableArray(validateNode(option, [].concat(_toConsumableArray(path), ['options', index]), [].concat(_toConsumableArray(ancestors), [node]))));
    });
  }
...
function getValue(value, state) {
  if ((typeof value === 'undefined' ? 'undefined' : _typeof(value)) === 'object' && value.fields) {
    return value.fields.reduce(function (acc, currentValue) {
      return acc + (0, _lodash2.default)(state, currentValue);
    }, 0);
  }
...
function buildIs(_ref10) {
  var field = _ref10.field,
      errorMessage = _ref10.errorMessage;

  return function (state) {
    if ((0, _lodash2.default)(state, field, false)) {
      return { valid: true, errors: [] };
    }
    ...
    return { valid: false, errors: [[{ field: field }, 'skal være sann']] };
  };
}
...
function validateSimpleExpression(expression) {
  ...
  switch (expression.operator) {
    ...
    case 'neq':
      ...
      if (_typeof(expression.value) === 'object' && !expression.value.field && !expression.value.fields) {
